...
POINT_FIG_NAMES = {
    "MI-DL": "colebrook_2000pts_midl_3d_logpi_data_points.png",
    "SI-DL": "colebrook_2000pts_sidl_3d_logpi_data_points.png",
    "Known [Re, k_s/D]": "colebrook_2000pts_known_3d_logpi_data_points.png",
    "Joint MI-DL": "colebrook_2000pts_joint_midl_3d_logpi_data_points.png",
}
...
def build_summary() -> tuple[pd.DataFrame, pd.DataFrame]:
    coordinates = pd.read_csv(SOURCE_DIR / "colebrook_coordinates.csv").head(N_SAMPLES)
    generated = pd.read_csv(SOURCE_DIR / "colebrook_generated_data.csv").head(N_SAMPLES)
    ...
    return summary, feature_rows, methods, pd.DataFrame(exponents), joint["audit"]
...
def plot_summary(summary: pd.DataFrame) -> None:
    ...
    fig.suptitle("Colebrook 2000 points, unified log-pi feature space", y=1.03)
    fig.tight_layout()
    fig.savefig(OUTPUT_DIR / "colebrook_2000pts_logpi_metric_summary.png", bbox_inches="tight")
    plt.close(fig)


def plot_summary_table(summary: pd.DataFrame) -> None:
    fig, ax = plt.subplots(figsize=(17.0, 5.4), dpi=220)
    ax.axis("off")
    fig.patch.set_facecolor("white")
    ax.text(
        0.5,
        0.96,
        "Colebrook 2000 points, log-pi comparison",
        ha="center",
        va="top",
        fontsize=18,
        weight="bold",
    ...
    for (r, c), cell in table.get_celld().items():
        ...
        if r == 0:
            ...
            text.set_fontsize(7.1 if c == 1 else 9.8)
            if c == 1:
                text.set_ha("left")
            if c == 0:
                text.set_weight("bold")
    fig.savefig(FIG_DIR / "colebrook_2000pts_summary_table.png", bbox_inches="tight", facecolor="white")
    plt.close(fig)
...
def plot_logpi_data_points_3d(feature_rows: pd.DataFrame, methods: list[dict]) -> list[Path]:
    ...
    written = []
    old_combined = FIG_DIR / "colebrook_2000pts_logpi_data_points.png"
    if old_combined.exists():
        old_combined.unlink()
    for old_pi_fig in FIG_DIR.glob("colebrook_2000pts_*_3d_pi_data_points.png"):
        old_pi_fig.unlink()
    for method in methods:
        name = method["method"]
        x = feature_rows[f"{name}_log_pi1"].to_numpy(float)
        y = feature_rows[f"{name}_log_pi2"].to_numpy(float)
        fig = plt.figure(figsize=(7.0, 5.6), dpi=240)
        ...
        ax.set_title(f"Colebrook 2000 data points - {name}")
...
def main() -> None:
    ...
    summary, feature_rows, methods, exponents, joint_audit = build_summary()
    summary_csv = OUTPUT_DIR / "colebrook_2000pts_midl_sidl_logpi_summary.csv"
    features_csv = OUTPUT_DIR / "colebrook_2000pts_logpi_coordinates.csv"
    exponents_csv = OUTPUT_DIR / "colebrook_2000pts_logpi_exponents.csv"
    audit_csv = OUTPUT_DIR / "colebrook_2000pts_joint_midl_audit.csv"
    ...
    print(f"Wrote {OUTPUT_DIR / 'colebrook_2000pts_logpi_metric_summary.png'}")
    print(f"Wrote {FIG_DIR / 'colebrook_2000pts_summary_table.png'}")
